...
def valid_swim(row):
    ...
    vis_starts = np.where((rawdata['stimParam4'][1:]==4) & (rawdata['stimParam4'][:-1]<4))[0] + 1
    vis_ends = np.where((rawdata['stimParam4'][1:]<4) & (rawdata['stimParam4'][:-1]==4))[0] + 1
    vis_starts = frame_tcourse[vis_starts].astype('int')
    vis_ends = frame_tcourse[vis_ends].astype('int')
    vis_ends = vis_ends[vis_ends>1]
...
if __name__ == "__main__":
    ...
    swim_xls_file.to_csv('depreciated/analysis_sections_GA_SOVO_SnFR.csv')