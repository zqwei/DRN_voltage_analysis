...
def single_x(voltr, window_length=41, win_=1001):
    ...
    voltr_ = roll_scale(voltr_, win_=win_) # nan value is skip by pandas by default
...
#         return None
    ...
    # remove part around the time of social water pouring
    voltrs_peak = np.argmax(voltrs, axis=-1)
    voltrs_peak_min = voltrs_peak.min() - 600 # remove 1 sec before
    voltrs_peak_max = voltrs_peak.max() + 2400 # remove 6 sec after
    voltrs_peak_min = voltrs_peak.min() - 600 # remove 1 sec before
    voltrs_peak_max = voltrs_peak.max() + 2400 # remove 6 sec after
    voltrs_new = voltrs.copy()
    voltrs_new[:, voltrs_peak_min:voltrs_peak_max] = np.nan
    
    spk_list, spkprob_list, spk1_list, spk2_list, voltr_list = voltr2spike_(voltrs_new, window_length, m)
    np.savez_compressed(f'{save_folder}/Voltr_spikes', voltrs=voltrs_new, \
                        spk=spk_list, spkprob=spkprob_list, spk1=spk1_list, \
                        spk2=spk2_list, voltr_=voltr_list, voltrs_old=voltrs)
