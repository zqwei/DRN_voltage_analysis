...
if __name__ == "__main__":
    if len(sys.argv)>1:
        ...
        for ablt_sovo_str_alt, ablt_len in zip(ablt_sovo_str_alt_set, ablt_len_set):
            for index, row in dat_xls_file.iterrows():
                ablation_pair = search_paired_data(row, dat_xls_file, ablt_len=ablt_len, ablt_sovo_str_alt=ablt_sovo_str_alt)
                ...
                if not os.path.isfile(save_folder+f'/finished_voltr{fext}.tmp'):
                    ...
                    x, y = align_components(row, ablt_len=ablt_len, ablt_sovo_str_alt=ablt_sovo_str_alt)
                    voltron_ablt(row, x, y, fext='', is_mask=True, ablt_sovo_str_alt=ablt_sovo_str_alt)
